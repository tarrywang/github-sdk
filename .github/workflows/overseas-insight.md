...
数据策略：**以联网深度研究为主**。RSS 种子源提供可靠基线，你需要在网络白名单内进行多轮联网抓取与交叉验证，
综合形成判断；不要只依赖固定 RSS 列表。
...
2. 规划本轮多轮联网检索方向：为每个品类×市场组合准备检索意图，例如「美妆 北美 爆品 趋势」「3C gadget trending Europe」
   「skincare TikTok Shop bestseller Southeast Asia」「饰品 配饰 跨境 热销」。
3. 标记 `fetchable: research-only` 的源（Amazon/TikTok/Google Trends）为「尽力而为」目标，不可达时走兜底。
...
1. 在白名单内对种子源与检索发现的新页面进行多轮抓取（贸易媒体报道、品类趋势、榜单解读等），交叉验证至少 2 个来源。
2. 重点补充「热门产品 / 潜力爆品」证据：品类、目标市场、价格带、核心卖点、为什么火。优先采用近 48 小时、可引用链接的信息。
3. 将联网研究得到的高价值信号并入工作信号集（与 `raw_signals.json` 一并参与后续聚类）。
4. 任何 research-only 目标不可达时，在最终报告「数据来源」处注明缺口，并以 RSS 基线继续，不要中断流程。
